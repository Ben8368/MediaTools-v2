...
- 本轮新增：前后端进度单位必须在展示边界显式归一；后端 `0..1` 与前端百分比 `0..100` 混用时，会把真实 5% 显示成 0.1% 一类假进度。
- 本轮新增：Windows 命令输出可能被系统语言本地化；解析 `netstat -e` 等状态指标时不能只认英文标签，至少要按计数器形态和目标本地化标签兜底。
- 本轮新增：下载后处理依赖“前后快照”时，测试桩的元数据探测命令不能提前创建输出文件，否则 Windows CI 可能把真实下载产物误判为已存在文件。
- 本轮新增：常驻服务里的 Windows 网络指标不能只依赖外部 CLI；`netstat` 可能受服务环境/文件锁/本地化影响，应优先使用系统 API，CLI 只做 fallback。
- 本轮新增：浏览器登录态读取需要用户显式选择和确认；工具只能提示用户自行退出对应浏览器，不能自动关闭用户已经打开的 Chrome/Safari/Firefox。
