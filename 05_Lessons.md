...
### L-044：常驻壳层不能直接暴露 Legacy 占位 API
- **时间：** 2026-06-26 10:26:52 +08:00
- **背景：** 右侧运行状态面板轮询 `getSystemMetrics`，但该函数仍是 Legacy 后端占位实现，导致用户在常驻面板看到“此功能需要旧版 MediaTools 后端，v2 尚未实现”的内部错误。
- **经验：** Legacy 壳层迁移时，任何默认可见或高频轮询组件都必须接入 v2 已实现端点或提供安静降级数据，不能直接调用 `v2NotReady` 占位函数。系统状态这类聚合视图可先由前端适配层组合 `/api/doctor`、`/api/fetch/tasks` 等稳定端点，真实 CPU/GPU/网络指标等后续再作为增量能力补齐。
- **状态：** 已采纳

### L-045：日志与通知入口应先提供轻量兼容数据
- **时间：** 2026-06-26 10:31:06 +08:00
- **背景：** 修复运行状态面板后，日志窗口仍调用 Legacy `fetchLogs` 占位函数，打开默认日志窗口时继续显示“旧版 MediaTools 后端未实现”的错误。
- **经验：** 壳层的日志、通知、未读数这类系统入口即使后端能力尚未完整实现，也应先提供轻量兼容层：可从 v2 任务记录生成基础日志行，或返回结构稳定的空态；清空、标记已读等操作在没有真实存储前可做幂等 no-op。这样用户看到的是“暂无日志/0 未读”，而不是内部迁移占位。
- **状态：** 已采纳

### L-046：绿灯完美应自动落地到 Git
- **时间：** 2026-06-26 10:40:01 +08:00
- **背景：** 用户希望后续 Audit Report 若为 `🟢 完美`，AI 不再停留在口头汇报，而是自动完成本地提交和远端推送。
- **经验：** 绿灯自动提交需要写入治理文档，但必须绑定客观验证和工作区边界：只提交本轮相关文件，保留无关未跟踪文件和用户改动；提交带对应 AI 的 `Co-Authored-By`；推送失败时明确说明。`🟡 可通行` 和 `🔴 阻断` 不自动提交，避免把仍需决策或修复的状态固化进远端。
- **状态：** 已采纳
