...
type PhotoshopMiniAiChatProps = {
  open: boolean
  onClose: () => void
  /** 附在每次 Agent 提问前的隐式上下文，不在界面展示 */
  taskContextLine: string | null
  /** 由快捷操作注入输入框；key 每次递增以触发写入 */
  composerSeed?: { key: number; text: string } | null
  onComposerSeedConsumed?: () => void
}

export function PhotoshopMiniAiChat({
  open,
  onClose,
  taskContextLine,
  composerSeed = null,
  onComposerSeedConsumed,
}: PhotoshopMiniAiChatProps) {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => [
    {
      ...
      role: 'assistant',
      content:
        '我是 Photoshop 自动化助手。你可以问扫描工单、文字层/智能对象、字体与导出路径，或让我根据当前工单与选中任务给操作建议。',
    },
  ])
  ...
  useEffect(() => {
    if (!open || !composerSeed) return
    setDraft(composerSeed.text)
    onComposerSeedConsumed?.()
    requestAnimationFrame(() => textareaRef.current?.focus())
  }, [open, composerSeed, onComposerSeedConsumed])
  ...
  async function send() {
    ...
    const contextBlock = taskContextLine ? `[Photoshop 上下文]\n${taskContextLine}\n\n` : ''
  ...
  return (
    <div className="dl-mini-ai ps-mini-ai" role="dialog" aria-label="Photoshop · AI 辅助对话">
      <header className="dl-mini-ai__head">
        <span className="dl-mini-ai__title">Photoshop 助手</span>
        ...
        <div className="dl-mini-ai__composer-inner">
          <div className="dl-mini-ai__input-wrap">
            <textarea
              ...
              rows={1}
              placeholder="描述排版问题、工单字段或执行报错…"
              value={draft}
              aria-describedby={draft.length > 0 ? 'ps-mini-ai-draft-stats' : undefined}
              title={
                draft
          ...
          {draft.length > 0 && (
            <div id="ps-mini-ai-draft-stats" className="dl-mini-ai__draft-stats" aria-live="polite">
